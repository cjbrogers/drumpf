...
class Round():
    ...
    def handle_player_turn(self, command, user_id):
        ...
        print("  User sending message: {}".format(
            self.bot.user_ids_to_username[user_id]))
        # waiting for first player after dealer to play a card
        if user_id != self.bot.player_turn_queue[0]:
            response = "Waiting for <@{}> to play a card.".format(
                current_username)
        elif user_id == self.bot.player_turn_queue[0]:
            print('  Received input from expected player: {}'.format(
                current_username))
            ...
            allowable_index = int(
                self.bot.current_game.current_round) - (int(self.bot.sub_rounds_played) + 1)
            print "  set allowable_index: {}".format(allowable_index)

            # the command must be between 0 and the largest index of the card
            # in the players hand
            if int(command) >= 0 and int(command) <= allowable_index:
                print("  Selected a valid card index")
                card_being_played = self.get_card_being_played(
                    user_id, int(command))

                if card_being_played == None:
                    self.bot.private_message_user(
                        user_id, "That's not a valid card index")
                    return
                card_value = None
                card_suit = None
                ...
                if self.bot.leading_suit != None:
                    ...
                    # a drumpf or a visible minority card or a tremendous card
                    # is always a valid play
                    if card_value.startswith("d_") or card_value.startswith("vm_") or card_value.startswith("t_"):
                        ...
                        # any card is valid, because player doesn't have the
                        # initial suit
                        self.handle_valid_card_played(card_being_played)
                    else:
                        self.bot.private_message_user(
                            user_id, "Sorry, you can't play that card")
                    ...
                    if card_value.startswith("d_") or card_value.startswith("t_"):
                        print("  {} played a Drumpf or Tremendous  Card first".format(
                            current_username))
                        ...
                        if self.bot.first_card_sub_round == 1:
                            ...
                            if "t_nasty" in card_value:
                                ...
                                self.trump.prompt_dealer_for_trump_suit(
                                    user_id)
                            else:
                                self.bot.leading_suit = "Any"
                                self.handle_valid_card_played(
                                    card_being_played)
                        else:
                            self.bot.leading_suit = "Any"
                            self.handle_valid_card_played(card_being_played)
                    elif card_value.startswith("vm_"):
                        print("  {} played a Visible Minority Card".format(
                            current_username))
                        # the sub round suit stays None until a player plays a
                        # suited card
                        self.handle_valid_card_played(card_being_played)
                    else:
    ...
    def handle_valid_card_played(self, card):
        ...
        if len(self.bot.player_turn_queue) == 0:
            ...
            msg = ">*<@{}> won this sub-round with a {}*\n\n".format(
                self.bot.winner_for_sub_round, helper_functions.emojify_card(self.bot.winning_sub_round_card))
            print " ", msg
            self.score.build_scoreboard(msg)
            self.score.update_scoreboard(self.bot.scoreboard)
            ...
            if self.bot.sub_rounds_played == self.bot.current_game.current_round:
                ...
                # initialize another turn queue cause there are more cards to
                # play
                ...
                while self.bot.player_turn_queue[0] != self.bot.winner_for_sub_round:
                    ...
                    # rotate player_turn_queue until the first player is the
                    # one who won
    ...
    def remove_card_from_players_hand(self, current_player_id, card_to_remove):
        ...
        for player in self.bot.current_game.players:
            if player.id == current_player_id:
                # we found the player, now remove the appropriate card from
                # his/her hand
