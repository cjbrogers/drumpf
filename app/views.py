...
# end of the Slack signin process, appending relevant user information
# including tokens into the db
...
def post_signin():
    ...
    values = {"user_id": user_id, "name": name, "access_token": access_token, "bot_access_token": bot_access_token,
              "bot_user_id": bot_user_id, "team_id": team_id, "team_name": team_name, "bot_im_id": im_id}
...
# completes the ouath add to slack process, adding relevant tokens to the
# database
...
def post_install():
    ...
    values = {"access_token": access_token,
              "bot_access_token": bot_access_token, "team_id": team_id}
    df = pd.DataFrame(values, index=[0])
    engine = models.get_engine()
